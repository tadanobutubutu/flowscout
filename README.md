...
<p align="center">
  ...
  <img src="https://img.shields.io/github/license/tadanobutubutu/flowscout?style=for-the-badge&color=blue" alt="License" />
  <img src="https://img.shields.io/badge/Accessibility-Grade%20A%20%28100%2F100%29-emerald?style=for-the-badge&logo=accessibility" alt="Accessibility Grade A" />
...
1. 🔍 **超高速リポジトリ検索**
   * 所有・参加している大量のリポジトリ（パブリック / プライベート）の中から、目的のプロジェクトをリアルタイム入力で一瞬で特定。
2. 🚨 **詳細なエラーログのスマホ特定**
   * 直コミットやPRで走ったワークフローをExpansionTileでスマートに展開。
   * どのジョブのどのステップでビルドエラーが起きたかをビジュアル化し、エラー理由まで完全に特定。
3. 🛡️ **セキュアな「スマート・ルーティング」OAuth連携**
...
*   **多言語化:** Crowdinとの統合機能（`crowdin.yml`）を完備しています。言語リソースは `lib/src/localization/` 配下のARBファイル（`app_ja.arb`, `app_en.arb`）で構成されています。
